...
/**
 * JavaScript required by the scoreboard/report code.
 *
...
var dw_Event={add:function(obj,etype,fp,cap){
    ...
    getTarget:function(e){
        e=dw_Event.DOMit(e);
        var tgt=e.target;
        if(tgt.nodeType!=1)tgt=tgt.parentNode;return tgt;
    }
}
...
var dw_Util;
if(!dw_Util)dw_Util={};
dw_Util.getImage=function(src){var img=new Image();img.src=src;return img;};
...
dw_Util.getCurrentStyle=function(el,prop){
    ...
    if(document.defaultView&&document.defaultView.getComputedStyle){
        ...
        if(!/^\d+(px)?$/i.test(val)&&/^\d/.test(val)){
            ...
            el.style.left=val||0;
            val=el.style.pixelLeft+"px";el.style.left=style;
...
dw_Util.getPageOffsets=function(el){
    var left=0,top=0;
    do{left+=el.offsetLeft;top+=el.offsetTop;}
...
var dw_Tooltip={
    tip:null,timer:0,hoverTimer:0,active:false,actuator:null,resetFlag:false,restored:true,init:function(){
        var _this=dw_Tooltip,el;
        if(_this.hasSupport()){
            _this.setProps(true);
            if(dw_Util.isTouchDevice()){
                if(!_this.supportTouch){return;}_this.forTouch=true;
            }
    ...
    plainWrap:function(str){return str;},
    ...
    calcPosCoords:function(e,tgt){
        ...
        var x=_this.evX,y=_this.evY;
        var maxX=_this.getMaxX(),maxY=_this.getMaxY();
        var xXd,yXd,dx,dy;
        var tx=x+_this.offX,ty=y+_this.offY;
        ...
        if(typeof e=='object'&&_this.foc){
            ...
            if(_this.focX<=maxX){tx=_this.focX;}
            else{tx=_this.focX=maxX;}
        }
        else if(_this.focX){
            tx=_this.focX;
        }
        if(tx<=maxX&&!_this.Left){x=tx;}
        else if(altx>=vp.scrollX&&(_this.jumpLeft||_this.Left)){x=altx;}
        else if((_this.jumpLeft&&spL)||_this.Left){x=vp.scrollX;xXd='Left';}
        else{x=maxX;xXd='Right';}
        if(ty<=maxY&&!_this.Above){y=ty;}
        else if(alty>=vp.scrollY&&(_this.jumpAbove||_this.Above)){y=alty;}
        else if((_this.jumpAbove&&spA)||_this.Above){y=vp.scrollY;yXd='Above';}
        else{y=maxY;yXd='Below';}
        if(xXd&&yXd){
            dx=(xXd=='Left')?vp.scrollX-altx:tx-maxX;dy=(yXd=='Above')?vp.scrollY-alty:ty-maxY;
            if(dx<=dy){x=(xXd=='Left')?altx:tx;}
            else{y=(yXd=='Above')?alty:ty;}
        }
        return{x:x,y:y}
    },
    adjust:function(){
    ...
    deactivate:function(e){
        var _this=dw_Tooltip;
        if(!_this.tip||!_this.active||_this.sticky){return;}
        e=e?e:window.event;
        if(e.type&&e.type=='mouseout'&&!dw_Util.mouseleave(e,_this.actuator)){return;}
        _this.clearTimer('timer');
        _this.clearTimer('hoverTimer');
        if(_this.hoverable){_this.hoverTimer=setTimeout(_this.hide,_this.hoverDelay);return;}
    ...
    handleOptions:function(e){
        var _this=dw_Tooltip;
        _this.coordinateOptions();
        if(_this.klass){_this.tip.className=_this.klass;}
        if(_this.hoverable){_this.tip.onmouseout=_this.tipOutCheck;_this.tip.onmouseover=function(){
            _this.clearTimer('hoverTimer');
            }
        }
        ...
        if(!_this.activateOnClick){dw_Event.add(_this.actuator,"click",_this.checkLinkClick);}
    },
 ...
 if(_this.forTouch||_this.activateOnClick||_this.sticky||_this.hoverable||_this.duration||_this.positionFn!=_this.positionRelEvent){
        ...
        if(_this.forTouch||_this.activateOnClick||_this.sticky){_this.hoverable=false;_this.duration=0;}
        if(_this.hoverable){_this.duration=0;}
    ...
    handleRestore:function(){
        var _this=dw_Tooltip;
        if(_this.followMouse){dw_Event.remove(document,'mousemove',_this.positionRelEvent,true);}
        else if(!(_this.forTouch||_this.activateOnClick)&&(_this.sticky||_this.duration)){
            dw_Event.remove(document,"mouseup",_this.checkDocClick,true);
        }
        if(!_this.forTouch){_this.tip.onmouseover=_this.tip.onmouseout=function(){};}
        if(!_this.activateOnClick&&_this.actuator){dw_Event.remove(_this.actuator,"click",_this.checkLinkClick);}
        if(_this.resetFlag){_this.setProps();}
        _this.resetRequest();
        _this.tip.innerHTML='';
        _this.active=false;
        _this.actuator=null;
        _this.tip.style.width='';
        _this.focX=0;_this.foc=false;
        _this.restored=true;
    },
    getTipClass:function(cls){
        if(!cls){return '';}
        var c='',classes=cls.split(/\s+/);
    ...
    checkForProps:function(obj){
        var _this=dw_Tooltip,list=_this.getPropsList(),prop;
        for(var i=0;list[i];i++){
            prop=list[i][0];
    ...
    tipOutCheck:function(e){
        var _this=dw_Tooltip,tip=this;
        if(dw_Util.mouseleave(e,tip)){_this.timer=setTimeout(_this.hide,_this.hideDelay);}
    },
    ...
    checkDocClick:function(e){
        ...
        if(tgt===_this.actuator||tgt===tip){return;}
    ...
    checkLinkClick:function(e){
        var _this=dw_Tooltip,tgt=_this.actuator,cur_href,tgt_href,cur,pg,pt;
        if(!(tgt.nodeName.toLowerCase()=='a')&&!(tgt.nodeName.toLowerCase()=='area')){return;}
        tgt_href=tgt.href;
        if(!tgt_href||!(tgt_href.lastIndexOf('#')==tgt_href.length-1)){return;}
        cur_href=location.href;
        pt=cur_href.indexOf('#');
        cur=(pt!=-1)?cur_href.slice(0,pt):cur_href;
        pt=tgt_href.indexOf('#');pg=(pt!=-1)?tgt_href.slice(0,pt):tgt_href;
        if(cur==pg){e=dw_Event.DOMit(e);e.preventDefault();}
    },
    clearTimer:function(timer){if(dw_Tooltip[timer]){clearTimeout(dw_Tooltip[timer]);dw_Tooltip[timer]=0;}},
    ...
    checkForActuator:function(e){
        var tgt=dw_Event.getTarget(e);
        if(!tgt){return;}
        var _this=dw_Tooltip,tipAct=null,ctr=0,msg='',id='',maxCnt=_this.maxLoops;
        do{
            if((tipAct=_this.getActuatorInfo(tgt))){
                ...
                switch(e.type){
                    case 'mouseover':
                        ...
                        if(_this.forTouch&&_this.activateOnClick){
                            dw_Event.add(tgt,'click',function(e){e.preventDefault();});
                        }
                        break;
                    case 'click':
                    ...
                    case 'focus':
                    case 'focusin':
                        if(_this.active&&tgt==_this.actuator){return;}
                        if(window.addEventListener){
                            dw_Event.add(tgt,'blur',_this.deactivate);
                        }
                        else if(window.attachEvent){dw_Event.add(tgt,'focusout',_this.deactivate);}
                        break;
                }
    ...
    getActuatorInfo:function(tgt){
        var _this=dw_Tooltip,cls=_this.getTipClass(tgt.className);
        if(!cls)return '';
        var vars=_this.content_vars||{},msg='',id='',el;
        id=(vars&&vars[cls])?cls:'';
        if(!id&&_this.content_source=='class_id'&&(el=document.getElementById(cls))){msg=el.innerHTML;}
        if(id||msg){return{msg:msg,id:id}}return false;
    },
    initHandlers:function(){
        ...
        if(!_this.forTouch){
            ...
            else{dw_Event.add(document,'mouseover',_this.checkForActuator,true);}
            dw_Event.add(document,"keydown",_this.checkEscKey,true);
            if(!_this.activateOnClick&&_this.activateOnFocus){
                if(window.addEventListener){dw_Event.add(document,'focus',_this.checkForActuator,true);}
                else if(window.attachEvent){dw_Event.add(document,'focusin',_this.checkForActuator);}
            }
        }
        else{dw_Event.add(document,'mouseover',_this.checkForActuator,true);}
        _this.ready=true;
...
dw_Tooltip.wrapWithCloseBox = function(str, caption) {
    ...
    msg = '<div class="topBar"><span class="closeBox" style="position:absolute; top:0; right:0; text-align:right;">';
    msg = msg + '<a href="javascript: void dw_Tooltip.hide()"' + css + '>' + box + '</a></span><span class="caption">';
    msg = msg + caption + '</span></div><div class="XtipContent">' + str + '</div>';
    return msg;
}
...
dw_Tooltip.wrapWithCloseLink = function(str, caption) {
    ...
    str += '<div class="close" style="text-align:center; margin:4px 0;">';
    str += '<a href="javascript: void dw_Tooltip.hide()">Close</a></div>';
    return str;
}
