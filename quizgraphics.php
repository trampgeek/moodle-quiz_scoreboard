...
if (!(has_capability('mod/quiz:manage', $contextinstance))) {
    echo "\n<br />".get_string('youmustbeauthorized', 'quiz_scoreboard');
    exit;
}
...
if (!($questiontext = $DB->get_record('question', array('id' => $questionid)))) {
    echo "\n<br />".get_string('youmustsubmitquestonid', 'quiz_scoreboard');
    exit;
}
...
if ($order) {
        ...
        if (strlen($value) > 0) {
            ...
            foreach ($values as $qansid) {
                if (isset($myx[$qansid])) {
                    ...
                    echo "\n<br />".get_string('somethingiswrongwithanswerid', 'quiz_scoreboard')." $qansid";
                }
            }
        }
    }
    ...
    echo "\n<br />".get_string('questionis', 'quiz_scoreboard').$questiontext->questiontext;
    $graphicurl = $CFG->wwwroot."/mod/quiz/report/scoreboard/graph.php";
    echo "\n<br /><img src=\"".$graphicurl.$graphinfo."&cmid=".$cm->id."\"></img>";
} else {
    echo "\n<br />";
    $quizgraphicsurl = $CFG->wwwroot."/mod/quiz/report/scoreboard/quizgraphics.php";
    if ($showstudents) {
        echo "<a href='".$quizgraphicsurl."?question_id=$questionid&quizid=$quizid&showstudents=0'>";
        echo get_string('hidenames', 'quiz_scoreboard')."</a>";
    } else {
        echo "<a href='".$quizgraphicsurl."?question_id=$questionid&quizid=$quizid&showstudents=1'>";
        echo get_string('shownames', 'quiz_scoreboard')."</a>";
    }
    echo "\n<br />".get_string('questionis', 'quiz_scoreboard').$questiontext->questiontext;
